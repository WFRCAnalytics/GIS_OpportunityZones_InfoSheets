...
cat(sprintf(
  "focal bbox (3857): xmin=%.0f ymin=%.0f xmax=%.0f ymax=%.0f\n\n",
  focal_bbox["xmin"],
  focal_bbox["ymin"],
  focal_bbox["xmax"],
  focal_bbox["ymax"]
...
cat(sprintf(
  "TRACT_BBOX:  xmin=%.0f ymin=%.0f xmax=%.0f ymax=%.0f\n",
  TRACT_BBOX["xmin"],
  TRACT_BBOX["ymin"],
  TRACT_BBOX["xmax"],
  TRACT_BBOX["ymax"]
...
cat(sprintf(
  "TRACT_BBOX WGS84: xmin=%.5f ymin=%.5f xmax=%.5f ymax=%.5f\n",
  TRACT_BBOX_WGS84["xmin"],
  TRACT_BBOX_WGS84["ymin"],
  TRACT_BBOX_WGS84["xmax"],
  TRACT_BBOX_WGS84["ymax"]
))
...
cat(sprintf(
  "counties CRS epsg=%s  xmin=%.5f  ymin=%.5f  xmax=%.5f  ymax=%.5f\n",
  ctys_crs,
  ctys_bbox["xmin"], ctys_bbox["ymin"],
  ctys_bbox["xmax"], ctys_bbox["ymax"]
))
# After safe_read_mvt revert: epsg=4326, values in degrees (~-112, ~40)
# If epsg=3857 with wrong coords (~-4975133): old source still in memory → re-source

# Data is in native 4326 — check for geographic CRS, not 3857
stopifnot("counties must be geographic (4326/CRS84) — re-source if failing" =
          is.na(ctys_crs) || ctys_crs == 4326L)

# Overlap check in 4326 degrees (TRACT_BBOX_WGS84 ≈ -112°, 40°)
...
cat(sprintf("TRACT_BBOX_WGS84 overlaps tile counties bbox: %s\n", overlaps))
...
for (si in c(4L, 2L, 3L, 6L, 5L, 7L, 1L, 0L)) {
  if (TEST_ZOOM < .ROAD_MIN_ZOOM[as.character(si)]) {
    next
  }
  rd <- .rsym(si)
  if (nrow(rd) == 0) {
  ...
  p <- p +
    ggplot2::geom_sf(
      data = rd,
      color = .C_ROAD_CAS,
      linewidth = .road_lw(TEST_ZOOM, si, "cas")
    )
}
...
if (nrow(interstates) > 0) {
  p <- p +
    ggplot2::geom_sf(
      data = interstates,
      color = .C_ROAD_CAS,
      linewidth = .road_lw(TEST_ZOOM, "0ir", "cas")
    )
  lf <- .road_lw(TEST_ZOOM, "0ir", "fil")
  if (lf > 0) {
    p <- p +
      ggplot2::geom_sf(data = interstates, color = .C_ROAD_FIL, linewidth = lf)
  }
}
cat(
  "\n── Step C: + roads ──────────────────────────────────────────────────────\n"
...
if (nrow(trax) > 0) {
  lw_cas <- .trax_lw(TEST_ZOOM, "cas")
  lw_fil <- .trax_lw(TEST_ZOOM, "fil")
  for (sym_v in unique(trax$map_symbol)) {
    tr <- trax[trax$map_symbol == sym_v, ]
    if (nrow(tr) == 0) {
      next
    }
    cv <- tr$trax_color[1]
    ...
    if (cv != .C_TRAX_DEF) {
      p <- p +
        ggplot2::geom_sf(
          ...
          color = cv,
          linewidth = lw_fil,
          linetype = .trax_lty(TEST_ZOOM, sym_v)
        )
    } else {
...
# ── Step E: + labels ────────────────────────────────────────────────────────
str_lbl <- extract_label_coords(.filter_street_labels(
  lbl_data$street,
  TEST_ZOOM
))
...
if (length(missing) > 0) {
  ...
  message(
    "Run libraries / prepare / load chunks in index.qmd, then re-run from here."
  )
...
focal_oz <- sf::st_transform(
  dplyr::filter(oz_tracts, GEOID == TEST_GEOID),
  map_crs_sf
)
other_oz <- sf::st_transform(
  dplyr::filter(oz_tracts, GEOID != TEST_GEOID),
  map_crs_sf
)
...
cat(sprintf(
  "wc=%d  other_oz=%d  exits=%d  stops=%d  sap=%d\n",
  nrow(wc_view),
  nrow(other_view),
  nrow(exits_view),
  nrow(stops_view),
