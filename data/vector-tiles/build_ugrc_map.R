...
library(sf)
library(ggplot2)
library(dplyr, warn.conflicts = FALSE) # explicit pkg::fn() used throughout; masking is intentional
library(showtext)
library(shadowtext)
...
.C_SP_MID <- "#B6BCBF"
.C_SP_BDR <- "#999391" # State Park z13+ inner border — JSON exact (was "#8C867E", issue #16)
...
# z < 11  (90m):  JSON spec = solid (no fill-opacity). Using 1.00 (full opacity)
#                 to match JSON exactly; county borders/fills render beneath hillshade.
...
.HS_ALPHA_TBL <- list(
  # JSON 90m layers have no fill-opacity → full opacity 1.0 (issue #4)
  `0` = c(lt11 = 1.00, b11_14 = 0.80, b14_15 = 0.40, ge15 = 0.24),
  `1` = c(lt11 = 1.00, b11_14 = 0.80, b14_15 = 0.40, ge15 = 0.24),
  `2` = c(lt11 = 1.00, b11_14 = 0.80, b14_15 = 0.40, ge15 = 0.24),
  `3` = c(lt11 = 1.00, b11_14 = 0.80, b14_15 = 0.56, ge15 = 0.40)
)
...
# Municipalities
.C_MUNI1_BDR <- "#807B79"
.C_M2_FILL <- rgb(153, 148, 138, 26, maxColorValue = 255) # 0.10 — JSON rgba(153,148,138,0.1)
# JSON alpha: outer 0.15, inner 0.20 — both below print threshold.
# Raised to 0.50 (outer) / 0.55 (inner) so the coloured halo reads in print
# while retaining the per-symbol purple/green/teal/mauve hue.
.C_M2_OUT <- list(
  "0" = rgb(144, 120, 161, 38, maxColorValue = 255), # purple  → 0.15
  "1" = rgb(156, 179, 134, 38, maxColorValue = 255), # green   → 0.15
  "2" = rgb(134, 179, 179, 38, maxColorValue = 255), # teal    → 0.15
  "3" = rgb(179, 134, 149, 38, maxColorValue = 255) # mauve   → 0.15
)
# sym 0 inner: z12-15 slightly different purple shade (exact JSON RGB, raised alpha)
.C_M2_IN_SYM0_LO <- rgb(160, 134, 179, 51, maxColorValue = 255) # z12-15 → 0.20
.C_M2_IN <- list(
  "0" = rgb(153, 128, 171, 51, maxColorValue = 255), # z15+ purple → 0.20
  "1" = rgb(156, 179, 134, 51, maxColorValue = 255), # green       → 0.20
  "2" = rgb(134, 179, 179, 51, maxColorValue = 255), # teal        → 0.20
  "3" = rgb(179, 134, 149, 51, maxColorValue = 255) # mauve       → 0.20
)
...
.C_RAIL <- "#B3AFAF"
.C_TRAIL_CAS <- "#ADA5A5"
# Trail label colours — distinct from park labels (issues #10, #17)
.C_TRAIL_LBL_TXT <- "#8C8989" # JSON text-color
.C_TRAIL_LBL_HALO <- rgb(230, 228, 225, 153, maxColorValue = 255) # rgba(230,228,225,0.60)
...
.C_WATER_LBL <- "#829599"
# Stream halo: rgba(230,226,218,0.55) — warm beige (issue #11); lake halo different below
.C_STM_HALO <- rgb(230, 226, 218, 140, maxColorValue = 255) # rgba(230,226,218,0.55) streams
# Lake halo: rgba(183,199,199,0.60) — blue-gray, different base colour from streams (issue #12)
.C_WATER_HALO <- rgb(183, 199, 199, 153, maxColorValue = 255) # rgba(183,199,199,0.60) lakes/GSL
.C_PARK_LBL <- "#8C8989" # parks, golf, cemeteries (JSON #8C8989, was "#8C8C77", issue #13-trail area)
# Park halo: rgba(194,204,173,0.30) golf/cemetery; 0.20 for parks (issue #9)
.C_PARK_HALO <- rgb(194, 204, 173, 77, maxColorValue = 255) # rgba(194,204,173,0.30) golf/cemetery
.C_PARK_HALO_LO <- rgb(194, 204, 173, 51, maxColorValue = 255) # rgba(194,204,173,0.20) parks
# Monument label: z8-13 uses lighter colour; z13+ uses darker (issue #13)
.C_MON_LBL <- "#807E7D" # z13+ (darker)
.C_MON_LBL_HI <- "#8C8A89" # z8-13 (brighter — JSON class 0-5 text-color)
.C_MON_HALO <- rgb(230, 228, 225, 128, maxColorValue = 255) # rgba(230,228,225,0.50)
.C_SKI_LBL <- "#8C8989"
# Ski area halo: rgba(230,227,225,0.55) — was rgba(242,239,237,0.55), completely wrong (issue #14)
.C_SKI_HALO <- rgb(230, 227, 225, 140, maxColorValue = 255) # rgba(230,227,225,0.55)
...
.ROAD_LW_SCALE <- 2 # global multiplier applied to all road casing + fill widths
...
.fetch_water <- function(bu, zoom) {
  list(
    ...
    # JSON Perennial Major (sym 0) starts at z6; Perennial (sym 1) at z11 (issue #6)
    streams = if (zoom >= 6) {
...
.fetch_buildings <- function(bu, zoom) {
  # JSON has no minzoom on Buildings; z≥14 gate is an intentional performance choice
  # (buildings are sparse/tiny at z12-13 on tract-scale maps). Change to z≥12 for
  # full spec fidelity (issue #30, documented limitation).
...
.fetch_labels <- function(lu, bu, zoom) {
  list(
    ...
    # WesternStates/label — class 0 z4-5, class 1 z6-8, class 2 z8+ (issue #23, was z>=8)
    west_lbl = if (zoom >= 4) {
...
# Perpendicular tick marks along railroad linestrings (JSON symbol-spacing 6.667px).
# Transforms to EPSG:3857 first to ensure metre-unit arithmetic.
.railroad_ties <- function(rail_sf, zoom) {
  if (nrow(rail_sf) == 0) {
    return(NULL)
  }
  rail_sf <- tryCatch(sf::st_transform(rail_sf, 3857), error = function(e) {
    rail_sf
  })
  spacing_m <- (6.667 / 4) * .px_to_m(zoom)
  half_m <- (2.5 / 3) * .px_to_m(zoom)
  result <- lapply(seq_len(nrow(rail_sf)), function(i) {
    geom <- sf::st_geometry(rail_sf[i, , drop = FALSE])[[1L]]
    if (inherits(geom, "MULTILINESTRING")) {
      sfc <- sf::st_cast(
        sf::st_sfc(geom, crs = sf::st_crs(rail_sf)),
        "LINESTRING"
      )
      if (length(sfc) == 0L) {
        return(NULL)
    ...
    targets <- seq(spacing_m / 2, total, by = spacing_m)
    rows <- lapply(targets, function(d) {
      idx <- max(
        1L,
        min(findInterval(d, cdist, rightmost.closed = TRUE), nrow(coords) - 1L)
      )
      t_p <- if (segs[idx] > 1e-9) (d - cdist[idx]) / segs[idx] else 0
      mx <- coords[idx, 1L] + t_p * (coords[idx + 1L, 1L] - coords[idx, 1L])
      my <- coords[idx, 2L] + t_p * (coords[idx + 1L, 2L] - coords[idx, 2L])
      dx <- coords[idx + 1L, 1L] - coords[idx, 1L]
      dy <- coords[idx + 1L, 2L] - coords[idx, 2L]
      len <- sqrt(dx^2 + dy^2)
      if (len < 1e-9) {
        return(NULL)
      }
      px <- -dy / len * half_m
      py <- dx / len * half_m
      data.frame(x = mx - px, y = my - py, xend = mx + px, yend = my + py)
    })
...
# Route label features by geometry type: LINESTRINGs sampled at spacing_px intervals;
# POINT/POLYGON features use centroid directly. ESRI MVT tiles often deliver road names
# as pre-placed POINTs even when the JSON spec says symbol-placement:line.
.label_pts_mixed <- function(sf_lbl, spacing_px, zoom) {
  if (nrow(sf_lbl) == 0L) {
    return(extract_label_coords(sf_lbl))
  }
  types <- sf::st_geometry_type(sf_lbl)
  is_line <- types %in% c("LINESTRING", "MULTILINESTRING")
  line_part <- if (any(is_line)) {
    extract_label_coords(.line_label_pts(sf_lbl[is_line, ], spacing_px, zoom))
  } else {
    extract_label_coords(sf_lbl[0L, ])
  }
  pt_part <- if (any(!is_line)) {
    extract_label_coords(sf_lbl[!is_line, ])
  } else {
    extract_label_coords(sf_lbl[0L, ])
  }
  rbind(line_part, pt_part)
}

# Shift a LINESTRING geometry perpendicular to its direction by offset_m meters.
# x_scale: Mercator latitude correction applied to the x-component only.
#   In EPSG:3857 the projection is conformal so projected metres are equal in
#   all directions, but when rendered via coord_sf the E-W (x) component of any
#   perpendicular shift appears wider than the N-S (y) component at mid-latitudes
#   because the bbox aspect ratio amplifies horizontal distances.  Passing
#   cos(lat) here (≈ 0.76 for Utah) corrects this empirical asymmetry.
.shift_perpendicular <- function(line_geom, offset_m, x_scale = 1) {
  coords <- sf::st_coordinates(line_geom)
  n <- nrow(coords)
  if (n < 2L) {
    return(line_geom + c(offset_m * x_scale, 0))
  }
  ...
  line_geom + c(-dy / len * offset_m * x_scale, dx / len * offset_m)
}
...
.prepare_trax <- function(trax, zoom) {
  ...
  # Mercator latitude correction for x-component: at Utah's latitude cos(lat) ≈ 0.76.
  # Derive from the EPSG:3857 y-coordinate of the geometry centroid using inverse
  # Mercator: lat = atan(sinh(y / R)).  Reduces the E-W shift so it matches the
  # visually correct N-S shift.
  mid_y <- tryCatch(
    {
      cents <- suppressWarnings(sf::st_coordinates(sf::st_centroid(sf::st_union(
        trax
      ))))
      cents[1L, 2L]
    },
    error = function(e) 4984000
  ) # fallback: SLC northing in EPSG:3857
  lat_rad <- atan(sinh(mid_y / 6378137))
  ew_scale <- cos(lat_rad)
...
# JSON TRAX widths use three discrete flat bands (not a ramp) — step lookup (issue #18).
.trax_lw <- function(zoom, type = "cas") {
  ...
  .lw(
    if (type == "cas") {
      c(2.667, 3.167, 3.667)[band]
    } else {
      c(1.333, 1.833, 2.333)[band]
    }
  )
}
...
# halo = NA means no halo (e.g. WesternStates/label has no text-halo in JSON — issue #15).
.lbl <- function(p, data, col, halo, sz, bgr, face = "bold", fam = "") {
  ...
  if (is.na(halo) || bgr <= 0) {
    # No halo: draw plain geom_text to avoid shadowtext artefacts with bg.r=0
    return(
      p +
        ggplot2::geom_text(
          data = dat,
...
.county_border_params <- function(zoom) {
  ...
  list(
    show = TRUE,
    c1 = scales::alpha("#736D67", 0.1),
    w1 = .lw(5.333),
    c2 = scales::alpha("#666361", 0.1),
    w2 = .lw(3.333),
    c3 = scales::alpha("#4D4B49", 0.3),
    w3 = .lw(0.667),
    lt = "C663"
  )
}
...
build_ugrc_map <- function(lon, lat, zoom, verbose = FALSE) {
  ...
  .v(sprintf(
    "  muni1(Carto:1)=%d  muni2(Carto:2)=%d  trax=%d  commuter_rail=%d  buildings=%d",
    nrow(muni$muni1),
    nrow(muni$muni2),
    nrow(transit$trax),
    nrow(transit$commuter_rail),
    nrow(buildings)
  ))
  ...
  # Label data
  cnty_filtered <- .filter_county_labels(lbl_data$county, zoom)
  cnty_pt <- if (nrow(cnty_filtered) > 0) {
    cnty_filtered[
      is.na(cnty_filtered$map_label_class) |
        cnty_filtered$map_label_class <= 2L,
    ]
  } else {
    cnty_filtered[0L, ]
  }
  cnty_ln <- if (nrow(cnty_filtered) > 0) {
    cnty_filtered[
      !is.na(cnty_filtered$map_label_class) &
        cnty_filtered$map_label_class == 3L,
    ]
  } else {
    cnty_filtered[0L, ]
  }
  cnty_lbl <- rbind(
    extract_label_coords(cnty_pt),
    .label_pts_mixed(cnty_ln, spacing_px = 1000, zoom)
  )
  ...
  hwy_filtered <- .filter_highway_labels(lbl_data$highway, zoom)
  hwy_shields <- if (nrow(hwy_filtered) > 0) {
    hwy_filtered[
      !is.na(hwy_filtered$map_label_class) &
        hwy_filtered$map_label_class %in% c(0L, 1L, 2L, 11L),
    ]
  } else {
    hwy_filtered[0L, ]
  }
  hwy_names <- if (nrow(hwy_filtered) > 0) {
    hwy_filtered[
      is.na(hwy_filtered$map_label_class) |
        !hwy_filtered$map_label_class %in% c(0L, 1L, 2L, 11L),
    ]
  ...
  hwy_shields_lbl <- extract_label_coords(hwy_shields)
  hwy_names_lbl <- .label_pts_mixed(hwy_names, spacing_px = 480, zoom)
  hwy_lbl <- rbind(hwy_shields_lbl, hwy_names_lbl)

  str_filtered <- .filter_street_labels(lbl_data$street, zoom)
  str_shields <- if (nrow(str_filtered) > 0) {
    str_filtered[
      !is.na(str_filtered$map_label_class) &
        str_filtered$map_label_class %in% c(0L, 1L, 2L),
    ]
  } else {
    str_filtered[0L, ]
  }
  str_names <- if (nrow(str_filtered) > 0) {
    str_filtered[
      is.na(str_filtered$map_label_class) |
        !str_filtered$map_label_class %in% c(0L, 1L, 2L),
    ]
  ...
  str_shields_lbl <- extract_label_coords(str_shields)
  str_names_lbl <- .label_pts_mixed(str_names, spacing_px = 1000, zoom)
  str_lbl <- rbind(str_shields_lbl, str_names_lbl)

  # Bounding box
  .valid_bbox <- function(sf_obj) {
    if (nrow(sf_obj) == 0L) {
      return(NULL)
    }
    ...
    bb <- suppressWarnings(tryCatch(sf::st_bbox(sf_obj), error = function(e) {
      NULL
    }))
  ...
  bb <- .valid_bbox(counties)
  if (is.null(bb)) {
    bb <- .valid_bbox(ground$western_states)
  }
  if (is.null(bb)) {
    bb <- .valid_bbox(hillshade)
  }
  if (is.null(bb)) {
    n <- 2^zoom
    lon0 <- coords$x / n * 360 - 180
    lon1 <- (coords$x + 1L) / n * 360 - 180
    .merc_lat <- function(yi) atan(sinh(pi * (1 - 2 * yi / n))) * 180 / pi
    c0 <- sf::st_coordinates(sf::st_transform(
      sf::st_sfc(sf::st_point(c(lon0, .merc_lat(coords$y + 1L))), crs = 4326),
      3857
    ))
    c1 <- sf::st_coordinates(sf::st_transform(
      sf::st_sfc(sf::st_point(c(lon1, .merc_lat(coords$y))), crs = 4326),
      3857
    ))
    bb <- c(xmin = c0[1], ymin = c0[2], xmax = c1[1], ymax = c1[2])
  }
  ...
  # Draw Utah tint from the single Utah polygon (ground$utah sym==0), NOT over every county
  # feature. Drawing over counties would stack the translucent tint N times (issue #31).
  utah_fill <- if (zoom >= 8) .C_UTAH_HI else .C_UTAH_LO
  utah_poly <- ground$utah[
    !is.na(ground$utah$map_symbol) & ground$utah$map_symbol == "0",
  ]
  p <- p +
    ggplot2::geom_sf(data = counties, fill = .C_CTY_FILL, color = NA)
  if (nrow(utah_poly) > 0) {
    p <- p + ggplot2::geom_sf(data = utah_poly, fill = utah_fill, color = NA)
  }
  ...
  if (nrow(ctrs) > 0) {
    ...
    if (nrow(ctr_500) > 0) {
      # JSON 500_1 (z10-12): 0.667px; 500 (z12+): 1.0px (issues #21)
      lw_500 <- if (zoom < 12L) .lw(0.667) else .lw(1.0)
      p <- p +
        ggplot2::geom_sf(
          data = ctr_500,
          color = .C_CTR_500,
    ...
    if (nrow(ctr_1000) > 0) {
      # JSON 1000_2 (z9-10): 0.667px; 1000_1 (z10-12): 1.0px; 1000 (z12+): 1.333px (issue #22)
      lw_1000 <- if (zoom < 10L) {
        .lw(0.667)
      } else if (zoom < 12L) {
        .lw(1.0)
  ...
  # 5. Municipalities Carto:2 — wide-under-narrow stroke stack on the original polygon.
  # Previously used .buf_sf() to offset strokes outward, but at tract-map scales the
  # buffer in paper units exceeds the stroke half-width, creating a visible gap between
  # the coloured halo and the Carto:1 dotdash. Mapbox GL line-offset places the stroke
  # centre at offset_px with width = 2×offset, so the inner edge always sits on the
  # boundary — no gap. Replicated here by drawing both strokes on the unmodified polygon:
  # outer (wider) first, inner (narrower) on top, then Carto:1 dotdash on top of both.
  # sym 0 outer colour differs at z<15 vs z15+ (JSON exact RGB, raised alpha for print).
  .m2_out0 <- if (zoom >= 15) {
    .C_M2_OUT[["0"]]
  } else {
  ...
  # 7. Bay Labels circle — JSON idx ≈93 (before county borders, issue #24)
  # Circle is invisible in JSON (circle-color rgba(0,0,0,0)) but positioned here for z-order.
  ...
  # Airport runway/taxiway lines — rgba(191,134,162,0.5), z≥10
  ...
  # 8. County borders — 3-stroke, 0.15x scale from JSON px
  if (cbs$show) {
  ...
  if (nrow(muni$muni1) > 0) {
    p <- p +
      ggplot2::geom_sf(
        ...
        fill = NA,
        color = .C_MUNI1_BDR,
        linewidth = .lw(1.5), # JSON 0.667px; raised to 1.5px for print visibility
  ...
  if (nrow(roads_raw$railroads) > 0) {
    ...
    tie_df <- .railroad_ties(roads_raw$railroads, zoom)
    if (!is.null(tie_df) && nrow(tie_df) > 0) {
      p <- p +
  ...
  # 11. Roads — CASING PASS (JSON order: sym4 → sym2 → sym3 → sym6 → sym5 → sym7 → sym1 → sym0)
  ...
  for (si in road_cas_order) {
    ...
    cas_col <- if (si == 5L && zoom < 11L) {
      # #12.3: sym5 z9-10 casing
      "#BABABA"
    } else if (si == 6L && zoom < 11L) {
      # #12.2: sym6 z10 hairline
      "#B0B0B0"
    } else {
      .C_ROAD_CAS
    }
  ...
  # 12. Roads — FILL PASS (JSON order: sym7 → sym5 → sym6 → sym4 → sym3 → sym2 → sym1 → sym0)
  for (si in road_fil_order) {
    ...
    lty <- if (si == 5L) "22" else "solid" # sym5 unpaved: JSON [2,2] → "22"
    p <- p +
      ggplot2::geom_sf(
        data = rd,
        color = .C_ROAD_FIL,
  ...
  # 13. Trails — JSON z14-15: outer 3px→0.45, fill 1.33px→0.20; z15+: 3.33→0.50, 1.67→0.25
  if (nrow(rec$trails) > 0) {
    ...
    # JSON fill dasharray: [2,2] at z14-15, [1.6,1.6] at z15+ → "22" / "11" (issue #27)
    tf_lty <- if (zoom < 15L) "22" else "11"
    p <- p +
      ggplot2::geom_sf(
        data = rec$trails,
        color = .C_ROAD_FIL,
  ...
  if (nrow(transit$commuter_rail) > 0) {
    ...
    p <- p +
      ggplot2::geom_sf(
        ...
        color = .C_FRONTRUNNER,
        linewidth = .lw(1.667),
        # JSON dasharray [4,2.4] — ggplot2 hex linetype digits are integers; [4,2] is
        # the closest representable value. Gap shortened by 0.4 units (issue #20, ggplot2 limit).
        linetype = "42"
      )
  }
  ...
  if (nrow(trax) > 0) {
    ...
    for (sym_v in unique(trax$map_symbol)) {
      ...
      if (cv != .C_TRAX_DEF) {
        ...
        # sym 10: JSON specifies 1.333px (= fill width), no casing (issue #19)
        p <- p + ggplot2::geom_sf(data = tr_sub, color = cv, linewidth = lw_fil)
      }
    }
  }
  ...
  # 15. Buildings — 3 JSON layers: shadow fill + main fill + border line
  if (nrow(buildings) > 0) {
    ...
    shadow_off <- (1.067 / 2) * .px_to_m(zoom)
  ...
  if (nrow(interstates) > 0) {
    ir0 <- interstates[
      !is.na(interstates$map_symbol) & interstates$map_symbol == "0",
    ]
    ir1 <- interstates[
      !is.na(interstates$map_symbol) & interstates$map_symbol == "1",
    ]
    if (nrow(ir0) > 0) {
      p <- p +
        ggplot2::geom_sf(
          data = ir0,
          color = .C_ROAD_CAS,
          linewidth = .road_lw(zoom, "0ir", "cas")
        )
      lf0 <- .road_lw(zoom, "0ir", "fil")
      if (lf0 > 0) {
        p <- p +
          ggplot2::geom_sf(data = ir0, color = .C_ROAD_FIL, linewidth = lf0)
      }
    }
    if (nrow(ir1) > 0 && zoom >= 11L) {
      ...
      if (lf1 > 0) {
        p <- p +
          ggplot2::geom_sf(data = ir1, color = .C_ROAD_FIL, linewidth = lf1)
      }
    }
  }
  ...
  if (nrow(poi$trailheads) > 0) {
    p <- p +
      ggplot2::geom_sf(
        ...
        size = 1.2,
        fill = .C_TRAIL_PT,
        color = scales::alpha(.C_TRAIL_PT, 0.7),
        stroke = 0.3
      )
  }
  if (nrow(poi$schools_he) > 0) {
    p <- p +
      ggplot2::geom_sf(
        data = poi$schools_he,
        shape = 21,
        size = .cr(1.83333), # JSON circle-radius 1.83333px (was 1.5, issue #26)
        fill = "transparent",
        color = .C_SCHOOL_HE_PT,
        stroke = .lw(0.866667) # JSON circle-stroke-width 0.866667px (was 0.6)
      )
  }
  if (nrow(poi$schools_k12) > 0) {
    p <- p +
      ggplot2::geom_sf(
        ...
        shape = 21,
        size = .cr(1.83333), # JSON circle-radius 1.83333px (was 1.2, issue #26)
        fill = "transparent",
        color = .C_SCHOOL_K12_PT,
        stroke = .lw(0.866667) # JSON circle-stroke-width 0.866667px (was 0.5)
      )
  }
  if (nrow(poi$healthcare) > 0) {
    p <- p +
      ggplot2::geom_sf(
        ...
        size = 1.2,
        fill = .C_HEALTH_PT,
        color = scales::alpha(.C_HEALTH_PT, 0.8),
        stroke = 0.3
      )
  }
  ...
  if (nrow(poi$city_dots) > 0) {
    cd <- poi$city_dots
    cd_s <- function(s) {
      cd[!is.na(cd$map_symbol) & cd$map_symbol == as.character(s), ]
    }
  ...
  if (nrow(lbl_data$gsl) > 0 && any(!is.na(lbl_data$gsl$map_label))) {
    ...
    # JSON halo only at z<10; no text-halo at z10+ (issue #25). Alpha 0.60 at z7-9.
    gsl_halo <- if (zoom < 10L) .C_WATER_HALO else NA_character_
    gsl_bgr <- if (zoom < 10L) .bgr(1.333, .sz_water_lbl(zoom, 0L)) else 0
    p <- .lbl(
      p,
      gsl_lbl,
      col = .C_WATER_LBL,
  ...
  if (nrow(lbl_data$lakes) > 0 && any(!is.na(lbl_data$lakes$map_label))) {
    ...
    if (nrow(lake_lbl) > 0 && any(!is.na(lake_lbl$map_label))) {
      lake_lbl$cls_i <- suppressWarnings(as.integer(lake_lbl$map_label_class))
      lake_lbl$cls_i[is.na(lake_lbl$cls_i)] <- 0L
      for (ci in unique(lake_lbl$cls_i)) {
        sub_l <- lake_lbl[lake_lbl$cls_i == ci & !is.na(lake_lbl$map_label), ]
        if (nrow(sub_l) == 0) {
          next
        }
        sz_lake <- .sz_water_lbl(zoom, ci)
        p <- .lbl(
          p,
          sub_l,
          col = .C_WATER_LBL,
  ...
  if (nrow(lbl_data$streams) > 0 && any(!is.na(lbl_data$streams$map_label))) {
    ...
    if (nrow(stm_lbl) > 0 && any(!is.na(stm_lbl$map_label))) {
      ...
      p <- .lbl(
        ...
        stm_lbl,
        col = .C_WATER_LBL,
        halo = .C_STM_HALO, # rgba(230,226,218,0.55) — warm beige (was lake colour, issue #11)
        sz = sz_stm,
  ...
  if (
    ...
    if (nrow(mon_lbl) > 0 && any(!is.na(mon_lbl$map_label))) {
      ...
      # JSON: z8-13 uses "#8C8A89", z13+ uses "#807E7D" (issue #13)
      mon_col <- if (zoom < 13L) .C_MON_LBL_HI else .C_MON_LBL
      p <- .lbl(
        p,
        mon_lbl,
        col = mon_col,
  ...
  # 15e. Park labels — z≥13; JSON halo rgba(194,204,173,0.20) — use _LO (issue #9)
  if (nrow(lbl_data$parks) > 0 && any(!is.na(lbl_data$parks$map_label))) {
    ...
    p <- .lbl(
      ...
      park_lbl,
      col = .C_PARK_LBL,
      halo = .C_PARK_HALO_LO, # rgba(194,204,173,0.20) for parks specifically
      sz = .sz_park_lbl(zoom),
  ...
  if (nrow(lbl_data$trails) > 0 && any(!is.na(lbl_data$trails$map_label))) {
    ...
    p <- .lbl(
      p,
      trail_lbl,
      col = .C_TRAIL_LBL_TXT, # "#8C8989" — JSON text-color (was .C_PARK_LBL="#8C8C77", issue #10)
      halo = .C_TRAIL_LBL_HALO, # rgba(230,228,225,0.60) — JSON exact (was .C_PARK_HALO, issue #10)
      sz = .sz_park_lbl(zoom),
      bgr = .bgr(1.333, .sz_park_lbl(zoom)),
      face = "bold",
      fam = .F_STREET
  ...
  # 15u. Western States label — line-placed, Helvetica Regular → arimo
  # JSON: class 0 z4-5, class 1 z6-8, class 2 z8+; text-color #828282, 10.667px (issue #23)
  if (
    !is.null(lbl_data$west_lbl) &&
      nrow(lbl_data$west_lbl) > 0 &&
      any(!is.na(lbl_data$west_lbl$map_label))
  ) {
    # Per-class zoom filter matching JSON minzoom/maxzoom
    wst_cls <- suppressWarnings(as.integer(lbl_data$west_lbl$map_label_class))
    wst_cls[is.na(wst_cls)] <- 2L
    lbl_data$west_lbl <- lbl_data$west_lbl[
      (wst_cls == 0L & zoom >= 4L & zoom < 6L) |
        (wst_cls == 1L & zoom >= 6L & zoom < 9L) |
        (wst_cls >= 2L & zoom >= 8L),
    ]
    ...
    if (nrow(wst_pts) > 0 && any(!is.na(wst_pts$map_label))) {
      ...
      p <- .lbl(
        ...
        wst_pts,
        col = .C_WEST_STATE,
        halo = NA_character_, # JSON has NO text-halo for WesternStates/label (issue #15)
        sz = sz_wst,
        bgr = 0,
        face = "plain",
        fam = .F_HWY
  ...
  # 16. County labels — Poller One, light text on gray halo
  ...
  # 17. City / town labels — wrap at 12 (10em Yu Gothic Bold)
  if (nrow(city_lbl) > 0 && any(!is.na(city_lbl$map_label))) {
    city_lbl$map_label <- .wrap_labels(city_lbl$map_label, 12L)
    city_lbl$cls_i <- suppressWarnings(as.integer(city_lbl$map_label_class))
    city_lbl$cls_i[is.na(city_lbl$cls_i)] <- 0L
    for (ci in unique(city_lbl$cls_i)) {
      sub_c <- city_lbl[city_lbl$cls_i == ci & !is.na(city_lbl$map_label), ]
      if (nrow(sub_c) == 0) {
        next
      }
      sz_c <- .sz_city(zoom, ci)
      p <- .lbl(
        p,
        sub_c,
        col = .C_CITY,
  ...
  # 18. Highway labels — per-class colour, zoom-interpolated size
  if (nrow(hwy_lbl) > 0 && any(!is.na(hwy_lbl$map_label))) {
    ...
    hwy_lbl$cls_i <- suppressWarnings(as.integer(hwy_lbl$map_label_class))
    hwy_lbl$cls_i[is.na(hwy_lbl$cls_i)] <- 4L
    for (ci in unique(hwy_lbl$cls_i)) {
      sub_lbl <- hwy_lbl[hwy_lbl$cls_i == ci & !is.na(hwy_lbl$map_label), ]
      if (nrow(sub_lbl) == 0) {
        next
      }
      cv <- sub_lbl$lbl_col[1L]
      sz_h <- .sz_hwy(zoom, ci)
      if (ci %in% c(0L, 1L, 2L, 11L)) {
        # Shield badge: coloured rectangle behind route number
        # cls 0 = Interstate (blue fill, white text), cls 1 = US Hwy (white fill, gray border),
        # cls 2/11 = State Hwy (white fill, gray border, rounded)
        badge_fill <- switch(as.character(ci), "0" = "#003087", "white")
        badge_col <- switch(as.character(ci), "0" = "white", "#555555")
        badge_lw <- switch(as.character(ci), "0" = 0.3, 0.5)
        badge_r <- switch(as.character(ci), "0" = 2, "1" = 4, 12)
        badge_pad <- switch(as.character(ci), "0" = 3, "1" = 3, 5)
        p <- p +
          ggplot2::geom_label(
            data = sub_lbl,
            mapping = ggplot2::aes(x = X, y = Y, label = map_label),
            ...
            fill = badge_fill,
            linewidth = badge_lw,
            label.r = grid::unit(badge_r, "pt"),
            label.padding = grid::unit(badge_pad, "pt"),
            fontface = "bold",
            family = .F_HWY
          )
      } else {
  ...
  if (nrow(str_lbl) > 0 && any(!is.na(str_lbl$map_label))) {
    str_lbl$cls_i <- suppressWarnings(as.integer(str_lbl$map_label_class))
    str_lbl$cls_i[is.na(str_lbl$cls_i)] <- 4L
    for (ci in unique(str_lbl$cls_i)) {
      sub_lbl <- str_lbl[str_lbl$cls_i == ci & !is.na(str_lbl$map_label), ]
      if (nrow(sub_lbl) == 0) {
        next
      }
      sz_s <- .sz_street(zoom, ci)
      if (ci %in% c(0L, 1L, 2L)) {
        badge_fill <- switch(as.character(ci), "0" = "#003087", "white")
        badge_col <- switch(as.character(ci), "0" = "white", "#555555")
        badge_lw <- switch(as.character(ci), "0" = 0.3, 0.5)
