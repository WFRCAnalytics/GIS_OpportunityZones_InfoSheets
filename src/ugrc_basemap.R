# src/ugrc_basemap.R
# UGRC LiteBase / LiteLabels / VectorHillshade basemap renderer.
# source() this file, call ugrc_map_init() once per session, then use
# build_ugrc_map(bbox, zoom). No library() calls; all refs are package::fn().
...
# Municipalities
.C_MUNI1_BDR <- "#807B79"
.C_M2_FILL <- rgb(153, 148, 138, 64, maxColorValue = 255) # 0.25
...
.C_M2_OUT <- list(
  "0" = rgb(144, 120, 161, 46, maxColorValue = 255), # purple  → 0.18
  "1" = rgb(156, 179, 134, 46, maxColorValue = 255), # green   → 0.18
  "2" = rgb(134, 179, 179, 46, maxColorValue = 255), # teal    → 0.18
  "3" = rgb(179, 134, 149, 46, maxColorValue = 255) # mauve   → 0.18
)
# sym 0 inner: z12-15 slightly different purple shade (exact JSON RGB, raised alpha)
.C_M2_IN_SYM0_LO <- rgb(160, 134, 179, 140, maxColorValue = 255) # z12-15 → 0.55
.C_M2_IN <- list(
  "0" = rgb(153, 128, 171, 140, maxColorValue = 255), # z15+ purple → 0.55
  "1" = rgb(156, 179, 134, 140, maxColorValue = 255), # green       → 0.55
  "2" = rgb(134, 179, 179, 140, maxColorValue = 255), # teal        → 0.55
  "3" = rgb(179, 134, 149, 140, maxColorValue = 255) # mauve       → 0.55
)
...
.county_border_params <- function(zoom) {
  ...
  list(
    show = TRUE,
    c1 = ggplot2::alpha("#736D67", 0.1),
    w1 = .lw(5.333),
    c2 = ggplot2::alpha("#666361", 0.1),
    w2 = .lw(3.333),
    c3 = ggplot2::alpha("#4D4B49", 0.3),
    w3 = .lw(0.667),
    lt = "C663"
  )
}
...
build_ugrc_map <- function(bbox, zoom, crs = 3857L, verbose = FALSE) {
  ...
  centre <- .bbox_centre_wgs84(bbox)

  # — Tile URLs —
  coords <- get_tile_xyz(centre$lon, centre$lat, zoom)
  .url <- function(svc) {
    sprintf(
  ...
  # 5c. GSL Bay Labels circle — JSON idx ≈93 (before county borders, issue #24)
  ...
  # 7. Municipalities Carto:2 — wide-under-narrow stroke stack on the original polygon.
  ...
  .m2_out0 <- if (zoom >= 15) {
    ...
    rgb(160, 134, 179, 46, maxColorValue = 255)
  }
  ...
  if (nrow(poi$trailheads) > 0) {
    p <- p +
      ggplot2::geom_sf(
        ...
        size = 1.2,
        fill = .C_TRAIL_PT,
        color = ggplot2::alpha(.C_TRAIL_PT, 0.7),
        stroke = 0.3
      )
  }
  ...
  if (nrow(poi$healthcare) > 0) {
    p <- p +
      ggplot2::geom_sf(
        ...
        size = 1.2,
        fill = .C_HEALTH_PT,
        color = ggplot2::alpha(.C_HEALTH_PT, 0.8),
        stroke = 0.3
      )
  }
